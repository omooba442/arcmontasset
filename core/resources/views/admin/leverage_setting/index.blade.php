                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline--danger ms-1 confirmationBtn"
                                            data-question="@lang('Are you sure to delete this leverage setting')?"
                                            data-action="{{route('admin.leverage.setting.delete',$game->id) }}">
...
<script>
    ...
    (function ($) {
        ...
        $('.addBtn').on('click', function (e) {
            let action = `{{ route('admin.leverage.setting.save') }}`;
            modal.find('form').trigger('reset');
            modal.find('.modal-title').text("@lang('Add Leverage Setting')")
            modal.find('form').prop('action', action);
            $(modal).modal('show');
        });

        $('.editBtn').on('click', function (e) {
            let action = `{{ route('admin.leverage.setting.save',':id') }}`;
            ...
            modal.find('.modal-title').text(`@lang('Update Leverage Setting')`);
            $(modal).modal('show');
        });
    })(jQuery);
</script>
