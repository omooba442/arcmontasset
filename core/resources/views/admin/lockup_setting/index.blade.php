                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline--danger ms-1 confirmationBtn"
                                            data-question="@lang('Are you sure to delete this lockup setting')?"
                                            data-action="{{route('admin.lockup.setting.delete',$game->id) }}">
...
<script>
    ...
    (function ($) {
        ...
        $('.addBtn').on('click', function (e) {
            let action = `{{ route('admin.lockup.setting.save') }}`;
            modal.find('form').trigger('reset');
            modal.find('.modal-title').text("@lang('Add Lockup Setting')")
            modal.find('form').prop('action', action);
            $(modal).modal('show');
        });

        $('.editBtn').on('click', function (e) {
            let action = `{{ route('admin.lockup.setting.save',':id') }}`;
            ...
            modal.find('.modal-title').text(`@lang('Update Lockup Setting')`);
            $(modal).modal('show');
        });
    })(jQuery);
</script>
