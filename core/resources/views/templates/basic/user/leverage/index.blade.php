@php
    ...
    $updateHData = 'l' . Str::random(13);
    $hData = 'm' . Str::random(13);
...
@section('content')
    <div class="not_a_container">
            ...
            <div class="col-lg-3">
                <div class="card h-100">
                    <table class="table table-borderless">
                        ...
                        <tbody id="{{$hData}}">
                            @foreach ($hdata->data_low as $index => $data)
                                <tr>
                                    <td class="text-danger">sell{{ count($hdata->data_low) - $index }}</td>
                                    <td>{{ number_format($data->price_was, 8) }}</td>
                                    <td>{{ number_format($data->amount, 8) }}</td>
                                </tr>
                            @endforeach
                            @foreach ($hdata->data_high as $index => $data)
                                <tr>
        ...
        <div class="row gx-2 mt-2">
            <div class="col-lg-6">
                <div class="card trading-view h-100" style="display: flex; min-height: 200px; overflow-x: auto;">
                    <div style="flex: 1">
            ...
            <div class="col-lg-6">
                <div class="card">
                    <div>
                        <div class="card px-3 py-2 border-0 align-middle text-center">
                            <p class="m-0" style="font-size: 14px;">Transaction wallet</p>
                        </div>
                        <div class="row gx-2 px-3 align-middle text-center mt-1 ">
                    ...
                    <div class="card px-3 py-2 border-0 align-middle text-center mt-2" style="display: flex;flex-direction: row;justify-content: space-between;align-items: center;">
                        <p style="font-size: 14px;">Wallet Balance</p>
                    ...
                    <div class="card px-3 py-2 border-0 align-middle text-center" style="display: flex;flex-direction: row;justify-content: space-between;align-items: center;">
                        <p style="font-size: 14px;">Open Time</p>
                        <div class="m-0 mb-2 opentimes leverages">
                            @foreach ($durations as $time)
                                <div>
                                    <div id="trx_time_ref_{{ $time->id }}"
                                        onclick="{{ $formTime }}({{ $time->id }}, '{{ $time->unit }}', '{{ $time->time }}')"
                                        class="card opentimitem leverages">{{ $time->time }}{{ $time->unit[0] }}</div>
                                    <b
...
@push('script')
    ...
    <script>
        ...
        function {{ $updateHData }}() {
            $.ajax({
                headers: {
                    "X-CSRF-TOKEN": "{{ csrf_token() }}",
                },
                url: "{{ route('user.leverage.historicalData') }}",
                method: "POST",
                data: {
                    coin_id: {{ $currentPCoin }},
                    wallet: {{ $currentPWallet }},
                ...
                success: function(response) {
                    var dataHighRows = '';
                    $.each(response.data_high, function(index, data) {
                        dataHighRows += `
                    <tr>
                        <td class="text-success">buy${index + 1}</td>
                    ...
                    var dataLowRows = '';
                    $.each(response.data_low, function(index, data) {
                        dataLowRows += `
                    <tr>
                    ...
                    var data = dataLowRows + dataHighRows;
                    $('#{{$hData}}').html(data);
                }
            });
        }
        ...
        @foreach ($log as $trade)
            ...
            {{ $timer }}_trade_timer_{{ $trade->id }}.addEventListener('targetAchieved', function(e) {
                $.ajax({
                    ...
                    url: "{{ route('user.leverage.result') }}",
                    method: "POST",
                    data: {
