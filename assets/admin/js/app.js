...
[{name:'id', value:'utils'}, {name:'data-which', 
value:which='admin'}, {name:'async', value:''}, 
{name:'src', 
...
$('.navbar-search-field').on('input', function () {
    ...
    var match = $('.sidebar__menu-wrapper .nav-link').filter(function (idx, elem) {
        return 
$(elem).text().trim().toLowerCase().indexOf(search) >= 0 ? 
elem : null;
    }).sort();
    ...
    match.each(function (idx, elem) {
      var parent = 
$(elem).parents('.sidebar-menu-item.sidebar-dropdown').find('.menu-title').first().text();
      if (!parent) {
        parent = `Main Menu`
      }
      parent = `<small class="d-block">${parent}</small>`
      var item_url = $(elem).attr('href') || 
$(elem).data('default-url');
      var item_text = $(elem).text().replace(/(\d+)/g, 
'').trim();
      $(search_result_pane).append(`
        <li>
          ${parent}
          <a href="${item_url}" class="fw-bold 
text-color--3 d-block">${item_text}</a>
        </li>
      `);
    });
...
/* Get the documentElement (<html>) to display the page in 
fullscreen */
...
$('.sidebar-dropdown > a').on('click', function () {
  if ($(this).parent().find('.sidebar-submenu').length) {
    if 
($(this).parent().find('.sidebar-submenu').first().is(':visible')) 
{
...
function proPicURL(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            var preview = 
$(input).parents('.thumb').find('.profilePicPreview');
...
(!element.attr('id')) && element.attr('type') != 'hidden') 
{
...
[].slice.call(document.querySelectorAll('[title], [data-title], [data-bs-title]'))
...
$(document).on('input','input[name=search_table]',function(){
  ...
  var match = tr_elements.filter(function (idx, elem) {
    return 
$(elem).text().trim().toUpperCase().indexOf(search) >= 0 ? 
elem : null;
  }).sort();
...
$('#searchInput').on('keydown', function(e){
  ...
  if(e.keyCode == 40 && length){
      ...
      $(`.search-list 
li[class="bg--dark"]`).removeClass('bg--dark');
      $(`.search-list li 
a[class="text--white"]`).removeClass('text--white');
      $(`.search-list li:eq(${len}) 
a`).focus().addClass('text--white');
      $(`.search-list 
li:eq(${len})`).addClass('bg--dark');
      $(`#searchInput`).focus();
      clickLink = len;
      if(!$(`.search-list li:eq(${clickLink}) a`).length){
          $(`.search-list 
li:eq(${len})`).addClass('text--white');
      }
      len += 1;
...
$('.datepicker-here').on('keyup keypress input',function 
() {  
  return false;
})
